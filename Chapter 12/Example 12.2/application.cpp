		...
		try
		{
			...
			{	
				//Render Drone
				{
					...
					D3DXMATRIX  matVP = view * proj;
					g_pEffect->SetMatrix("matVP", &matVP);
					g_pEffect->SetVector("lightPos", &lightPos);
					D3DXVECTOR4 b(cameraPos.x, cameraPos.y, cameraPos.z, 0.0f);
					g_pEffect->SetVector("cameraPos", &b);
