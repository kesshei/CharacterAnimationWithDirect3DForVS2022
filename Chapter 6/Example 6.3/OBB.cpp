...
bool OBB::Intersect(OBB& b)
{
	...
	D3DXVECTOR4 vPosB;
	D3DXVECTOR3 pos1 = (b.m_pos - m_pos);
	D3DXVec3Transform(&vPosB, &pos1, &r1);
...
bool OBB::Intersect(Ray& ray)
{
	...
	D3DXVECTOR3 d = -m_size;
	return D3DXBoxBoundProbe(&d, &m_size, &org, &dir) == TRUE;
}
...
Ray OBB::GetContactPoint(Ray& ray)
{
	...
	for (int i = 0; i < 6; i++)
	{
		...
		{
			D3DXVECTOR3 r;
			D3DXVECTOR3 d = (org + dir * 1000.0f);
			if (D3DXPlaneIntersectLine(&r, &planes[i], &org, &d) != NULL)
			{
				numPlanes++;
