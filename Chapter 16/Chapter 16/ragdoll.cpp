...
RagDoll::RagDoll(const char fileName[], D3DXMATRIX &world) : SkinnedMesh()
{
...
void RagDoll::InitRagdoll()
{
	...
	D3DXVECTOR3 ee = D3DXVECTOR3(0.0f, 0.0f, 1.0f);
	D3DXQuaternionRotationAxis(&q2, &ee, D3DX_PI * -0.5f);
...
void RagDoll::UpdateSkeleton(Bone* bone)
{
	...
	if(bone->m_pObb != NULL)
	{
		...
		D3DXMATRIX rot;
		D3DXQUATERNION xx = bone->m_pObb->GetRotation(bone->m_originalRot);
		D3DXMatrixRotationQuaternion(&rot, &xx);
