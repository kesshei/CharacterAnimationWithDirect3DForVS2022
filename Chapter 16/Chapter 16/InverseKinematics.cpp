...
InverseKinematics::InverseKinematics(SkinnedMesh* pSkinnedMesh)
{
	...
	if(m_pHeadBone != NULL)
	{
		...
		D3DXVECTOR4 vec;
		D3DXVECTOR3 b = D3DXVECTOR3(0, 0, -1);
		D3DXVec3Transform(&vec, &b, &toHeadSpace);
...
void InverseKinematics::UpdateArmIK()
{
	...
	D3DXVECTOR3 bbb = D3DXVECTOR3(0, 0, -1);
	ApplyArmIK(bbb, target);
}
...
{ 
	...
	D3DXVECTOR3 c = (-nmlStartToJoint);
	float currentJointAngle = acosf(D3DXVec3Dot(&c, &nmlJointToEnd));
