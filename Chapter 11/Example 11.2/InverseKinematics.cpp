...
InverseKinematics::InverseKinematics(SkinnedMesh* pSkinnedMesh)
{
	...
	if(m_pHeadBone != NULL)
	{
		...
		D3DXVec3Transform(&vec, &a, &toHeadSpace);
...
void InverseKinematics::UpdateArmIK()
{
	...
	D3DXVECTOR3 c = D3DXVECTOR3(0, 0, -1);
	ApplyArmIK(c, target);
}
...
{ 
	...
	D3DXVECTOR3 b = (-nmlStartToJoint);
	float currentJointAngle = acosf(D3DXVec3Dot(&b, &nmlJointToEnd));
